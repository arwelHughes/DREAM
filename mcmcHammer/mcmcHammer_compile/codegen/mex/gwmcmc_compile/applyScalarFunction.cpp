                         ...
                         ::coder::array<real_T, 2U> &z1)
{
  ...
  real_T d;
  real_T s;
  int32_T applyScalarFunction_numThreads;
  int32_T b;
  int32_T eint;
  int32_T k;
  int32_T ub_loop;
  ...
  b_st.prev = &st;
  b_st.tls = st.tls;
  z1.set_size(&bj_emlrtRTEI, sp, 1, x.size(1));
  st.site = &rd_emlrtRSI;
  ...
  applyScalarFunction_numThreads =
      emlrtAllocRegionTLSs(sp->tls, static_cast<boolean_T>(omp_in_parallel()),
                           omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel num_threads(applyScalarFunction_numThreads) private(      \
    d, absx, s, S, R, eint, b) firstprivate(emlrtHadParallelError)
  {
    try {

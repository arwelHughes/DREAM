function [stop,fid_1, fid_2] = dream_check(DREAMPar,Par_info,Meas_info)
% Check for setup errors

% Assign stop to be No
stop = 'no';

fname_suffix = 'checks_';

% open an output file for diagnostics
fid_1 = fopen([fname_suffix,'_DREAM_diagnostics.txt'],'w');

% open an output file for diagnostics
fid_2 = fopen([fname_suffix,'_DREAM_warnings.txt'],'w');
fprintf(fid_2,'-------------- DREAM warning file --------------\n');
...
if DREAMPar.N < (2 * DREAMPar.delta) + 1
    ...
    fprintf(evalstr); fprintf(fid_2,evalstr);
    % Stop DREAM
    stop = 'yes';
end

function [Sequences,X,Z,out,fx] = RAT_dream_zs(MCMCPar,Extra,ParRange,Measurement)
...
% % open an out file with warnings
...
if nargin < 3
    ...
    ParRange.minn = -Inf * ones(1,MCMCPar.n); 
    ParRange.maxn = Inf * ones(1,MCMCPar.n);
end
...
CR = zeros(floor(Nelem/MCMCPar.steps),MCMCPar.nCR+1);
AR = zeros(floor(Nelem/MCMCPar.steps),2);
R_stat = zeros(floor(Nelem/MCMCPar.steps),MCMCPar.n+1);
out = struct('CR',CR,'AR',AR,'R_stat',R_stat);
out.AR(1,1:2) = [MCMCPar.seq -1];


%Initialise 'iteration'    
iteration = 2; 
% Initialize Iter 
Iter = MCMCPar.seq; iloc = 1; T = 0;
...
%     coder.varsize('oCR', [Inf Inf], [1 1]);
%     coder.varsize('oAR', [Inf Inf], [1 1]);
%     coder.varsize('oR_stat', [Inf Inf], [1 1]);
% 
%     oCR = []; oAR = []; oR_stat = [];
...
%    elseif strcmp(MCMCPar.prior,'PRIOR')
        ...
        % TODO - Make a priorFun for this part....
        % Comment out the eval for now for compile reasons....
%         for qq = 1:MCMCPar.n
...
%         end
    ...
    % Save N_CR in memory
    out.CR(1,1:MCMCPar.nCR+1) = [Iter pCR];

    % Compute the R-statistic of Gelman and Rubin
    [out.R_stat(1,1:MCMCPar.n+1)] = [Iter Gelman(Sequences(1,1:MCMCPar.n,1:MCMCPar.seq),MCMCPar)];

else % If a restart run is being done: just load the out from the previous ongoing trial

      % TODO - Fix the reload functionality
%     load DREAM_ZS.mat; 
%     %load GWF1P81R04.mat
...
%         fprintf(evalstr); fprintf(fid,evalstr);    
    ...
    for gen_number = 1:MCMCPar.steps
        ...
        xold = X(1:MCMCPar.seq,1:MCMCPar.n); 
        log_p_xold = X(1:MCMCPar.seq,MCMCPar.n + 2);

        % Without replacement draw rows from Z for proposal creation
        R = randsample(MCMCPar.m, 2 * MCMCPar.DEpairs * MCMCPar.seq); 
        Zoff = Z(R,1:MCMCPar.n);
        ...
        if (T == MCMCPar.T)
            ...
            iloc = iloc + 1; 
            thisSample = reshape(X',1,MCMCPar.n+2,MCMCPar.seq);
    ...
    out.AR(iteration,1:2) = [Iter 100 * totaccept/(MCMCPar.steps * MCMCPar.seq)];

    % Store Important Diagnostic information -- Probability of individual crossover values
    out.CR(iteration,1:MCMCPar.nCR+1) = [Iter pCR];
    ...
    [out.R_stat(iteration,1:MCMCPar.n+1)] = [Iter Gelman(Sequences(start_idx:end_idx,1:MCMCPar.n,1:MCMCPar.seq),MCMCPar)];
...
% Start with CR
out.CR = out.CR(1:iteration-1,1:size(pCR,2)+1);
% Then R_stat
out.R_stat = out.R_stat(1:iteration-1,1:MCMCPar.n+1);
% Then AR
out.AR = out.AR(1:iteration-1,1:2);
% Then Sequences
...
% If five out arguments are requested then return fx
if nargout == 5
    if strcmp(MCMCPar.modout,'Yes')

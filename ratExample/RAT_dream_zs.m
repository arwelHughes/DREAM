function [Sequences,X,Z,out_CR,out_R_stat,out_AR,fx] = RAT_dream_zs(MCMCPar,Extra,ParRange,Measurement)
...
%         end
    ...
    % Save N_CR in memory
    out_CR(1,1:MCMCPar.nCR+1) = [Iter pCR];

    % Compute the R-statistic of Gelman and Rubin
    [out_R_stat(1,1:MCMCPar.n+1)] = [Iter Gelman(Sequences(1,1:MCMCPar.n,1:MCMCPar.seq),MCMCPar)];
...
%             end
    ...
    out_AR(iteration,1:2) = [Iter 100 * totaccept/(MCMCPar.steps * MCMCPar.seq)];

    % Store Important Diagnostic information -- Probability of individual crossover values
    out_CR(iteration,1:MCMCPar.nCR+1) = [Iter pCR];
    ...
    [out_R_stat(iteration,1:MCMCPar.n+1)] = [Iter Gelman(Sequences(start_idx:end_idx,1:MCMCPar.n,1:MCMCPar.seq),MCMCPar)];
...
% Start with CR
% - not sure what's going on with out_CR.... bit of a hack here...

%out_CR = [];
out_CR = out_CR(1:iteration-1,1:size(pCR,2)+1);
...
out = struct('CR',out_CR,'R_stat',out_R_stat,'AR',out_AR);

% Test....
% out_CR = 1; out_AR = 1; out_R_stat = 1;
...
%out = struct('CR',out_CR,'AR',out_AR,'R_stat',out_R_stat);
